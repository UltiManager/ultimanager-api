...
def test_save_unverified_email(_, mock_email_address_qs):
    ...
    """
    email = EmailAddress(address="test@example.com", is_verified=False)
...
def test_save_verified_email(_, mock_email_address_qs):
    ...
    """
    email = EmailAddress(address="test@example.com", is_verified=True)
    mock_email_address_qs.get.return_value = email
...
@mock.patch("account.api.serializers.email_utils.send_email")
def test_save_missing_email(mock_send_email, mock_email_address_qs):
    ...
    their account.
    """
    mock_email_address_qs.get.side_effect = EmailAddress.DoesNotExist
    ...
    assert mock_email_address_qs.get.call_args[1] == {"address": data["email"]}
    assert verification is None
